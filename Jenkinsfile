pipeline {
    ...
    stages {
        ...
        stage('Build-Image') {
            steps {
                sh "KAS_MACHINE=${params.MACHINE} KAS_TARGET=${params.IMAGE} kas build --force-checkout --update kas-ausweisapp.yml"
        ...
        stage ('Build-SDK') {
            ...
            steps {
               sh "KAS_MACHINE=${params.MACHINE} KAS_TARGET=${params.IMAGE} KAS_TASK=populate_sdk kas build kas-ausweisapp.yml"
